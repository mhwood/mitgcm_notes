...
def create_bathymetry_file(config_dir, bathy_file):

    print('Creating an nc file which stores the wet grids')
    ...
    wet_grid_C = df.create_3D_wet_grid(bathy_grid, delR, hFac='C', hFacMin=0.3, hFacMinDr=1.0)
    print('   - Calculating the hFacS wetgrid')
    wet_grid_S = df.create_3D_wet_grid(bathy_grid, delR, hFac='S', hFacMin=0.3, hFacMinDr=1.0)
    print('   - Calculating the hFacW wetgrid')
    wet_grid_W = df.create_3D_wet_grid(bathy_grid, delR, hFac='W', hFacMin=0.3, hFacMinDr=1.0)
    ...
    output_file = os.path.join(config_dir,  'input', 'wet_grids.nc')
...
if __name__ == '__main__':
    ...
    create_bathymetry_file(config_dir, bathy_file)